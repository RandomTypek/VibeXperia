...
int sensors_poll_context_t::batch(int handle, int flags, int64_t period_ns, int64_t timeout) {
    ...
    if (local_handle >= 0 && v1) {
        if (halIsAPILevelCompliant(this, handle, SENSORS_DEVICE_API_VERSION_1_1)) {
            ...
            // nicki fix: the stock clamp had three bugs that broke streaming on
            // the legacy v0 blob (all sensors stuck, no events, EINVAL from
            // setDelay -> SensorService "batch failed / activate called without
            // batch"): (1) it indexed global_sensors_list by the MODULE index
            // (get_module_index), not the sensor's index, so it always read
            // sensor[0]'s delays; (2) the blob reports maxDelay==0 / minDelay==0,
            // and clamping `period_ns = maxDelay` ZEROED the rate, making
            // setDelay(0) fail -EINVAL; (3) it passed the GLOBAL handle to
            // setDelay instead of the local one. Fix: find the sensor by handle,
            // clamp only when min/maxDelay are meaningful (>0), convert minDelay
            // us->ns, and forward the LOCAL handle.
            int64_t minDelay_ns = 0, maxDelay_ns = 0;
            for (int i = 0; i < global_sensors_count; i++) {
                if (global_sensors_list[i].handle == handle) {
                    minDelay_ns = (int64_t)global_sensors_list[i].minDelay * 1000LL;
                    maxDelay_ns = (int64_t)global_sensors_list[i].maxDelay * 1000LL;
                    break;
                }
            }
            if (minDelay_ns > 0 && period_ns < minDelay_ns) {
                period_ns = minDelay_ns;
            } else if (maxDelay_ns > 0 && period_ns > maxDelay_ns) {
                period_ns = maxDelay_ns;
            }
...
static void lazy_init_sensors_list() {
    ...
    for (std::vector<hw_module_t*>::iterator it = sub_hw_modules->begin();
        ...
        for (int i = 0; i < module_sensor_count; i++) {
            ...
            // nicki: sub_hw_versions is only populated by open_sensors(), and is
            // deleted+NULLed at the end of this function. If get_sensors_list() is
            // ever called before open() (or a second time), sub_hw_versions->at()
            // would deref NULL / throw -> SIGSEGV in system_server. Default to the
            // oldest API version (0) when the sub-HAL version is not yet known; the
            // legacy nicki blob reports version 0 anyway.
            int version = 0;
            if (sub_hw_versions != NULL) {
                std::unordered_map<hw_module_t *, int>::iterator vit =
                        sub_hw_versions->find(*it);
                if (vit != sub_hw_versions->end()) {
                    version = vit->second;
                }
            }
